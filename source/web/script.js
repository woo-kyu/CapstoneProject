...
function updateSequentialData() {
    ...
    strokeData.forEach((stroke, index) => {
        stroke.forEach((point, i) => {
            eventLog = `Event ${index * 1000 + i}: x=${point.x}, y=${point.y}, Vector: (${point.dx}, ${point.dy})<br>` + eventLog;
        });
    });
...
//
// function drawReplicaCanvas() { // used coordinate data
//     replicaCtx.clearRect(0, 0, replicaCanvas.width, replicaCanvas.height);
//     strokeData.forEach(stroke => {
//         if (stroke.length === 0) {
//             return;
//         }
//         replicaCtx.beginPath();
//         replicaCtx.moveTo(stroke[0].x, stroke[0].y);
//         for (let i = 1; i < stroke.length; i++) {
//             replicaCtx.lineTo(stroke[i].x, stroke[i].y);
//         }
//         replicaCtx.stroke();
//     });
// }